// api/admin/jornadas/[id].js - CRUD operations for specific jornada
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      try {
        ...
        const result = await client.query(`
          ...
          WHERE id = $6
          RETURNING *
        `, [week_number, start_date, end_date, is_current, is_completed, jornadaId]);
    ...
    if (req.method === 'DELETE') {
      // Delete jornada (with cascade handling)
      const client = await pool.connect();
      ...
      try {
        ...
        // Delete the jornada
        await client.query('DELETE FROM jornadas WHERE id = $1', [jornadaId]);
